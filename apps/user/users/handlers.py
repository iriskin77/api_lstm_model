...
@router_user.patch("/")
async def update_user(id: int, item: UserUpdate, current_user: UserBase = Depends(get_current_user_from_token)):
    ...
    try:
        updated_user_id = await services.update_user(id=id, params_to_update=params_to_update)
...
@router_user.delete("/")
async def user_delete(id: int, current_user: UserBase = Depends(get_current_user_from_token)):
    ...
    try:
        deleted_user_id = await services.delete_user(id=id)
